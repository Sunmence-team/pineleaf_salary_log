    ...
    onSubmit: async (values, { resetForm, setSubmitting }) => {
      ...
      try {
        const payload = {
          ...
          company_branch: "Lagos HQ",
          address: "Plot 4, Maitama",
          state: "Abuja",
          country: "Nigeria",
        };

        const response = await axios.post(`${API_URL}/employers`, payload, {
          headers: {
  ...
  useEffect(() => {
    const resolveAccount = async () => {
      ...
      if (accountNumber.length === 10 && selectedBankCode) {
        setIsResolving(true);
        try {
          const resolvedData = await resolveAccountNumber(accountNumber, selectedBankCode);
  ...
  return (
    <div className="px-4 md:px-6">
      <div className="w-full rounded-xl bg-white p-6 flex flex-col gap-8">
        <form onSubmit={formik.handleSubmit} className="grid md:grid-cols-2 grid-cols-1 gap-x-6 gap-y-4">
          {/* ================= PERSONAL INFO ================= */}
          ...
          <div className="flex flex-col gap-2">
            ...
            {formik.touched.employmentType && formik.errors.employmentType && (
              <p className="text-sm text-red-600">{formik.errors.employmentType}</p>
            )}
          </div>

          {/* Employment Date */}
          <div className="flex flex-col gap-2">
            <label htmlFor="employmentDate" className="text-sm font-medium">Date Employed</label>
            <input
              type="date"
              name="employmentDate"
              id="employmentDate"
            ...
            {formik.touched.employmentDate && formik.errors.employmentDate && (
              <p className="text-sm text-red-600">{formik.errors.employmentDate}</p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="account_name" className="text-sm font-medium">
              Account Name {isResolving && <span className="text-blue-600">(Resolving...)</span>}
            </label>
            <input
              type="text"
              id="account_name"
              ...
              readOnly
              placeholder={isResolving ? "Resolving account name..." : "Auto-filled after account number"}
              className="py-2 indent-3 border border-gray-300 rounded-md focus:outline-none bg-gray-50 cursor-not-allowed"
            />
            {formik.touched.account_name && formik.errors.account_name && (
              <p className="text-sm text-red-600">{formik.errors.account_name}</p>
            )}
          </div>

          {/* Salary */}
          <div className="flex flex-col gap-2">
            <label htmlFor="salary_amount" className="text-sm font-medium">Estimate Pay (₦)</label>
            <input
              type="number"
              name="salary_amount"
              id="salary_amount"
            ...
            {formik.touched.salary_amount && formik.errors.salary_amount && (
              <p className="text-sm text-red-600">{formik.errors.salary_amount}</p>
            )}
          </div>
